...
import { Wallet, CreditCard, Film, ShoppingBag, UtensilsCrossed, MoreHorizontal, Plus, Trash2, Edit2 } from 'lucide-react';
...
export default function Wants() {
  ...
  const getColorForCategory = (key: string) => {
    const colorMap: { [key: string]: string } = {
      savingsAccount: 'bg-green-100 text-green-600',
      emi: 'bg-red-100 text-red-600',
  ...
  return (
    <div className="p-6">
      <div className="max-w-7xl mx-auto">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900 mb-2">Wants Breakdown</h1>
          <p className="text-gray-600">Manage your discretionary spending and lifestyle expenses</p>
        </div>

        {/* Overview Cards */}
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          <div className="bg-white p-6 rounded-lg shadow-md border-l-4 border-green-500">
            <h3 className="text-lg font-semibold text-gray-900 mb-2">Total Wants Budget</h3>
            <p className="text-3xl font-bold text-green-600">₹{wantsAmount.toLocaleString()}</p>
            <p className="text-sm text-gray-500 mt-1">{state.wantsPercent}% of salary</p>
          </div>
          ...
          <div className="bg-white p-6 rounded-lg shadow-md border-l-4 border-blue-500">
            ...
            <p className="text-sm text-gray-500 mt-1">{((total / wantsAmount) * 100).toFixed(1)}% allocated</p>
          </div>
          ...
          <div className={`bg-white p-6 rounded-lg shadow-md border-l-4 ${difference >= 0 ? 'border-orange-500' : 'border-red-500'}`}>
            ...
            <p className="text-sm text-gray-500 mt-1">{difference >= 0 ? 'Available' : 'Over budget'}</p>
          </div>

          <div className="bg-white p-6 rounded-lg shadow-md border-l-4 border-purple-500">
            <h3 className="text-lg font-semibold text-gray-900 mb-2">Categories</h3>
            <p className="text-3xl font-bold text-purple-600">{Object.keys(state.wantsCategories).length}</p>
            <p className="text-sm text-gray-500 mt-1">Lifestyle categories</p>
          </div>
        </div>
          ...
          <div className="bg-white p-6 rounded-lg shadow-md">
            ...
            <div className="space-y-6">
              {Object.keys(state.wantsCategories).map((key) => {
                ...
                const percentage = ((amount / wantsAmount) * 100);
                        ...
                        {editingCategory === key ? (
                          ...
                          <div className="flex items-center justify-between">
                            <div>
                              ...
                              <p className="text-sm text-gray-500">Discretionary spending category</p>
                            </div>
                            <div className="flex items-center space-x-2">
                              <button
            ...
            <div className="mt-6 p-4 bg-gray-50 rounded-lg">
              ...
              <div className="flex justify-between items-center mt-2">
                ...
                <span className="text-sm text-gray-900">₹{wantsAmount.toLocaleString()}</span>
              </div>
              <div className="flex justify-between items-center">
        ...
        <div className="bg-gradient-to-r from-green-50 to-emerald-50 p-6 rounded-lg">
          <h2 className="text-xl font-semibold text-gray-900 mb-4">💡 Smart Wants Management</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              <h3 className="font-medium text-gray-900 mb-2">Priority Levels</h3>
            ...
            <div>
              <h3 className="font-medium text-gray-900 mb-2">Smart Spending</h3>
              <ul className="text-sm text-gray-600 space-y-1">
